<script lang="ts">
    ...
    let options = $derived(
        $eventsLayer === 'ex'
        ? ["general", "scaleX", "scaleY", "text", "color"] satisfies (ExtendedEventTypeName | "general")[]
        : ["general", "moveX", "moveY", "rotate", "alpha", "speed", "easing", "bpm"] satisfies (Exclude<keyof typeof EventType, ExtendedEventTypeName> | "general")[])
    ...
    const restore = () => {
        ...
        if (activatedEditor.autoRangeEnabled) {
            rangeInput.value = "auto";
        } else {
            let value = (activatedEditor as NumericEventCurveEditor).valueRange.join(", ");
            if (activatedEditor.timeGridInterval) {
                value += `, ${activatedEditor.timeGridInterval}`;
            }
